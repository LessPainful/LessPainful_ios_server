...
- (NSDictionary *)JSONResponseForMethod:(NSString *)method URI:(NSString *)path data:(NSDictionary*)data {
    ...
    if (query != nil) {
        ...
        if ([result count] >0) {
            ...
            NSArray* baseEvents = [Resources eventsFromEncoding:base64Events];
            
            targetView = v;
            self.events = [Resources transformEvents:baseEvents 
        ...
        self.events = [Resources eventsFromEncoding:base64Events];
    }
...
//NSLog(@"PLAY Events:\n%@",self.events);    
    ...
    if (base64Prototype) {
        NSArray* protoEvents = [Resources eventsFromEncoding:base64Prototype];
...
-(void) play:(NSArray *)events {
    _done = NO;
    [[Recorder sharedRecorder] load: self.events];
